...
describe ApplicationController do
  ...
  describe "Signup Page" do
    ...
    it 'redirects to error page if user tries to sign up without with an invalid email' do
      visit '/users/signup'
      fill_in "name", :with => "Garfield"
  ...
  describe 'dashboard (user show) page' do
    it 'shows all a single user\'s projects and supplies' do
      user = User.create(:name => "becky567", :email => "starz@aol.com", :password => "kittens")
      project1 = Project.create(:name => "GGS", :project_type => "event", :date => "September 1, 2019", :attendees => 100, :supplies => 200.00)
      get "/users/#{user.id}"
